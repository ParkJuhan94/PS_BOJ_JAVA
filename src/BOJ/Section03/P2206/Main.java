...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        System.setIn(new FileInputStream("src/BOJ.Section03/P2206/input.txt"));
        ...
        while (!q.isEmpty()) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                if(map[nr][nc] == '0'){
                    if(!cur.destroyed && !visited[nr][nc][0]) { // 부순 벽이 여태까지 없었으면
                        q.add(new Loc(nr, nc, nextCnt, false));
                        visited[nr][nc][0] = true;
                    }else if(cur.destroyed && !visited[nr][nc][1]){ // 벽을 한번 부순 적이 있으면
